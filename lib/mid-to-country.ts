...
// Source: ITU Maritime Identification Digits table

const MID_TABLE: Record<number, [string, string]> = {
  // [country, flag emoji]
  201: ["Albania", "🇦🇱"],
  202: ["Andorra", "🇦🇩"],
  203: ["Austria", "🇦🇹"],
  ...
  603: ["Angola", "🇦🇴"],
  605: ["Algeria", "🇩🇿"],
  607: ["Saint Paul & Amsterdam Islands", "🇫🇷"],
...
export function lookupFlag(mmsi: number): { country: string; emoji: string } {
  const mid = Math.floor(mmsi / 1000000);
  const entry = MID_TABLE[mid];
  if (entry) return { country: entry[0], emoji: entry[1] };
  return { country: "Unknown", emoji: "🏴" };
}
